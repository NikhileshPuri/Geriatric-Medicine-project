...
class ResNet1D(nn.Module):
    def __init__(self, n_classes=2):
        super().__init__()
        self.layer1 = ResidualBlock1D(1, 16)
        self.layer2 = ResidualBlock1D(16, 32)
        self.layer3 = ResidualBlock1D(32, 64)
...
def create_weighted_sampler(y_train, oversample_factor=2):
...
def eval_probs(model, loader, device):
    ...
    return np.array(trues), np.array(probs_all)
...
def main():
    ...
    parser.add_argument('--threshold_search', action='store_true', help='Automatically search best AF threshold to maximize precision')
    ...
    sampler = create_weighted_sampler(y_train, oversample_factor=2)
    ...
    device = args.device
    model = ResNet1D(n_classes=2).to(device)
    ...
    for ep in range(args.epochs):
        ...
        yv, pv_probs = eval_probs(model, val_loader, device)
        pv = apply_threshold(pv_probs, 0.5)
    ...
    # ---- Threshold search to maximize AF precision ----
    best_threshold = 0.5
    if args.threshold_search:
        thresholds = np.linspace(0.55, 0.75, 21)  # conservative thresholds
        yv, pv_probs = eval_probs(model, val_loader, device)
        prec_best = 0.0
        for t in thresholds:
            pv = apply_threshold(pv_probs, t)
            prec = precision_score(yv, pv)
            if prec > prec_best:
                prec_best = prec
                best_threshold = t
        print(f"Best AF probability threshold for maximum precision: {best_threshold:.2f}")

    # ---- Test evaluation ----
    yt, pt_probs = eval_probs(model, test_loader, device)
    pt = apply_threshold(pt_probs, best_threshold)
    ...
    print(f"Used AF probability threshold: {best_threshold:.2f}")
